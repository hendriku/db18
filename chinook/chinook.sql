...
-- Aufgabe 4
CREATE OR REPLACE VIEW employee_sales_v AS (
SELECT e.EMPLOYEEID, e.FIRSTNAME, e.LASTNAME, SUM(NVL(subE.TOTAL, 0)) AS "REVENUE", subE.SYEAR AS "YEAR"
FROM EMPLOYEE e JOIN (
   SELECT e1.EMPLOYEEID, e1.REPORTSTO, c.TOTAL, c.SYEAR FROM EMPLOYEE e1 LEFT JOIN (
        SELECT c1.CUSTOMERID, c1.SUPPORTREPID, i.TOTAL, TO_CHAR(i.INVOICEDATE, 'yyyy') AS "SYEAR"
...
WHERE NOT subE.SYEAR IS NULL
GROUP BY e.EMPLOYEEID, e.FIRSTNAME, e.LASTNAME, subE.SYEAR
)
ORDER BY e.EMPLOYEEID, subE.SYEAR;
